...
export default function buildDefines() {
   ...
   return {
      "defines.env.APP_VERSION": JSON.stringify(childProcess
         .execSync('./bin/version.sh')
         .toString().replace('\n', '')),
      "defines.env.THEME": JSON.stringify(usedTheme),
      "defines.env.THEME_VERSION": JSON.stringify(childProcess
         .execSync(usedTheme ? `cd src/themes/${usedTheme} && ../../../bin/version.sh` : './bin/version.sh')
         .toString().replace('\n', '')),
      ...
      'defines.env.THEME_COMMIT': JSON.stringify(
         (() => {
            try {
               return usedTheme && fs.existsSync(`./src/themes/${usedTheme}/config.json`) ? childProcess
                  .execSync(`cd src/themes/${usedTheme} && git rev-parse --short HEAD`)
                  .toString().replace('\n', '') : '';
            } catch (error) {
               return 'no commits available';
            }
